...
import { ShareVaultModalContent } from '../ShareVaultModalContent'
...
jest.mock('../CreateOrEditVaultModalContent', () => {
  const R = require('react')
  return {
    CreateOrEditVaultModalContent: (props: Record<string, unknown>) =>
      R.createElement('div', {
...
jest.mock('../ShareVaultModalContent', () => {
  const R = require('react')
  return {
    ShareVaultModalContent: () =>
...
describe('VaultSelector', () => {
  ...
  it('opens create vault modal when create is clicked', () => {
    ...
    expect(modal.type).toBe(CreateOrEditVaultModalContent)
  ...
  it('skips createInvite when invite target already matches inviteData.vaultId, then opens share modal', async () => {
    ...
    expect(shareNode.type).toBe(ShareVaultModalContent)
  })
